...
loginAndMigrate()
    ...
    // Migrate parking areas
    console.log('Migrating parking areas...')
    for (const parking of SEED_PARKING_AREAS) {
      const { error } = await supabase
        .from('parking_areas')
        .insert({
          id: parking.id,
          ...
          allocated_spaces: parking.allocatedSpaces,
          location: parking.location,
          linked_venue_ids: parking.linkedVenueIds,
          created_at: new Date().toISOString()
      })
      
      if (error) {
        console.error('Error inserting parking area:', parking.name, error)
      } else {
        console.log('✓ Parking area migrated:', parking.name)
      }
    }
